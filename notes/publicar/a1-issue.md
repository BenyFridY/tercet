...
#   (the import at server.py:87 runs inside create_payment_wrapper itself)
```
...
The snippets above reproduce NC-1 and NC-2 directly. We also keep an offline pytest
harness (no facilitator, no network) where each confirmed non-conformance is encoded
as `xfail(strict=True)` — the suite goes red the day the bug is fixed. Happy to
contribute it as regression tests alongside a fix PR. Environment: Windows 11,
Python 3.13, `x402==2.20.0`, `mcp==2.0.0`, Node 24, `@x402/mcp@2.23.0`.
Verified against `main` @ `dd927a2`.
